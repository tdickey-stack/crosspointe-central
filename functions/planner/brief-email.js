...
function normalizeRecipients(value) {
  ...
  if (recipients.length > MAX_RECIPIENTS) fail(`An announcement brief can be sent to at most ${MAX_RECIPIENTS} recipients at once.`);
...
function normalizeAttachment(value) {
  ...
  if (!base64 || base64.length > MAX_PDF_BASE64_LENGTH || !/^[A-Za-z0-9+/]+={0,2}$/.test(base64)) {
    fail("The attached announcement brief PDF is missing or too large.");
  }
...
export function normalizePlannerBriefEmailPayload(payload) {
  ...
  return {
    ...
    brief: {
      title: text(briefSource.title, 120) || "Announcement Brief",
      startDate,
      endDate,
      announcementTypes: Array.isArray(briefSource.announcementTypes) ?
        ...
        [],
      closingScriptTitle: text(briefSource.closingScript, 3000) ?
        text(briefSource.closingScriptTitle, 120) || "One Next Step" : "",
      closingScript: text(briefSource.closingScript, 3000),
      entries,
...
export function buildPlannerBriefEmailText(payload) {
  ...
  const lines = [
    ...
    "",
    payload.message || "Here is the latest Sunday announcement brief from Central.",
    "",
  ...
  const scriptEntries = brief.entries.filter((entry) => entry.sampleAnnouncement);
  if (scriptEntries.length || brief.closingScript) {
    lines.push("SUNDAY ANNOUNCEMENT SCRIPT", "");
    scriptEntries.forEach((entry) => {
      lines.push(entry.name, markdownToPlainText(entry.sampleAnnouncement), "");
    });
    if (brief.closingScript) {
      lines.push(brief.closingScriptTitle || "One Next Step", markdownToPlainText(brief.closingScript), "");
    }
  }
  lines.push("The complete announcement brief is attached as a PDF.");
  lines.push("Generated by CrossPointe Central Planner.");
  return lines.join("\n");
}
...
export function buildPlannerBriefEmailHtml(payload) {
  ...
  const scriptEntries = brief.entries.filter((entry) => entry.sampleAnnouncement);
  const scriptSections = scriptEntries.map((entry) => `
    <div style="padding:16px 0;border-top:1px solid #e5e7eb;">
      <h3 style="margin:0 0 8px;font-size:17px;line-height:1.3;color:#18181b;">${escapeHtml(entry.name)}</h3>
      <div style="color:#52525b;font-size:14px;line-height:1.6;">${renderMarkdownHtml(entry.sampleAnnouncement)}</div>
    </div>`).join("");
  const closingSection = brief.closingScript ? `
    <div style="padding:16px 0;border-top:1px solid #e5e7eb;">
      <h3 style="margin:0 0 8px;font-size:17px;line-height:1.3;color:#18181b;">${escapeHtml(brief.closingScriptTitle || "One Next Step")}</h3>
      <div style="color:#52525b;font-size:14px;line-height:1.6;">${renderMarkdownHtml(brief.closingScript)}</div>
    </div>` : "";
  const combinedScript = (scriptSections || closingSection) ? `
    <div style="margin:22px 0 0;padding:18px;border:1px solid #fecaca;border-left:5px solid #ef3e2d;border-radius:12px;background:#fffafa;">
      <div style="margin:0 0 12px;font-size:11px;font-weight:800;letter-spacing:.1em;color:#ef3e2d;">SUNDAY ANNOUNCEMENT SCRIPT</div>
      ${scriptSections}${closingSection}
    </div>` : "";
  return `<!doctype html>
  <html><body style="margin:0;padding:0;background:#f4f4f5;font-family:Arial,sans-serif;color:#27272a;">
    ...
    <div style="max-width:680px;margin:0 auto;padding:24px 14px;">
      ...
      <div style="padding:24px;background:#ffffff;border:1px solid #e4e4e7;border-top:0;">
        <p style="margin:0 0 20px;font-size:15px;line-height:1.6;color:#3f3f46;">${escapeHtml(payload.message || "Here is the latest Sunday announcement brief from Central.")}</p>
        ...
        ${cards}
        ${combinedScript}
        <p style="margin:20px 0 0;color:#71717a;font-size:12px;line-height:1.5;">The complete announcement brief is attached as a PDF. Generated by CrossPointe Central Planner.</p>
      </div>
    </div>
  </body></html>`;
}
